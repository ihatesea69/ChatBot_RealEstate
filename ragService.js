  ...
  async _loadConfigFromSecrets() {
    try {
      ...
      if (config.useRemoteEmbedding !== undefined) {
        ...
        console.log(
          `Remote embedding is ${
            this.useRemoteEmbedding ? "enabled" : "disabled"
          }`
        );
      }

      console.log(
        "Successfully loaded RAG configuration from AWS Secrets Manager"
      );
    } catch (error) {
      console.warn(
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (fileExt === ".txt") {
          ...
          console.log(
            `Skipping unsupported file: ${file} (only .txt supported in simple mode)`
          );
        }
      }
